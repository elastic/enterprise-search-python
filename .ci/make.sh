...
product="elastic/enterprise-search-python"
...
if [[ "$CMD" == "assemble" ]]; then
  ...
  docker run \
    --rm -v $repo/.ci/output:/code/enterprise-search-python/dist \
    $product \
    /bin/bash -c "python /code/enterprise-search-python/utils/build-dists.py $VERSION"

  # Verify that there are dists in .ci/output
	if compgen -G ".ci/output/*" > /dev/null; then

	  # Tarball everything up in .ci/output
    cd $repo/.ci/output && tar -czvf enterprise-search-python-$VERSION.tar.gz * && cd -
